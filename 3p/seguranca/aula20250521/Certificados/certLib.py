...
def carregaCertificado(file, formato='PEM', show=False):
    ...
    if show:
        ...
        print('Inicio Validade: ', cert.not_valid_before_utc)
        print('Fim Validade: ', cert.not_valid_after_utc)
...
def carregaCSR(file, formato='PEM', show=False):
    ...
    if show:
        ...
        print('Chave Pública: ', csr.public_key())
        print('Algoritmo Assinatura: ', csr.signature_hash_algorithm)
...
if __name__ == "__main__":
    ...
    help = '''
    -----------------------------------------------------------------------------
    AÇÃO FEITA PELA CA (Autoridade Certificadora)   
    Cria um certificado auto-assinado, isto é Emissor == Requerente 
    Este certificado precisa estar armazenado previamente nos computadores (TRUSTED)
    ...
    certCA = geraAutoAssinado(priCA, pubCA, dn, 'certCA.pem') 
    certCA = carregaCertificado("certCA.pem", show=True)
    # execute ** openssl x509 -in certCA.pem -text **

    help = '''
    -----------------------------------------------------------------------------
    AÇÃO FEITA PELO REQUERENTE (O servidor ou uma pessoa)
    Gera um CSR (Certificate Signing Request)   
    É uma solicitação de um requerente para que a CA assine o certificado
    Contém o nome e a chave pública do requerente, e é assinado pelo próprio requerente
    Digite <ENTER> para continuar
    -----------------------------------------------------------------------------
    '''
    input(help)
    ...
    -----------------------------------------------------------------------------
    AÇÃO FEITA PELA CA (Autoridade Certificadora)
    Assina o CSR do requerente para criar um certificado  
    A CA insere seu nome e sua assinatura feita com sua chave privada
    Digite <ENTER> para continuar
    -----------------------------------------------------------------------------
    '''
    input(help)
    ...
    help = '''
    -----------------------------------------------------------------------------
    AÇÃO FEITA PELO CLIENTE (browser ou outro aplicativo)
    Verifica se a Assinatura da CA que está no certificado é verdadeira    
    Esta operação necessita que o computador do cliente já tenha o certificado da CA
    Digite <ENTER> para continuar
    -----------------------------------------------------------------------------
    '''
    input(help)
    if verificaCertificado(certEU, certCA):
